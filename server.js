...
var server = http.createServer(function(request, response){
    ...
    }else if(urlJson.pathname=="/member/edit"){//회원 정보 보기를 요청하면..
        update(request, response);
    }else if(urlJson.pathname=="/category"){//동물 구분 정보 보기를 요청하면..
...
function getDetail(request, response){
    ...
    con.query(sql, function(error, record, fields){
        if(error){
            console.log("sql 연동 실패: ", error);
        }else{
            fs.readFile("./detail.ejs", "utf-8", function(error, data){
                if(error){
                    console.log("상세보기 로드 실패: ", error);
...
function del(request, response){
    ...
    //error, fields     (insert, update, delete)
    //error, record, fields(select)
    con.query(sql, function(error, fileds){
        if(error){
    ...
    request.on("data", function(param){
       ...
       con.query(sql, [
                ...
                if(error){
                    ...
                    var msg = "<script>";
                    msg+="alert('수정되었습니다.');";
                    msg+="location.href = '/member/detail?member2_id="+ postParam.member2_id+"';";
                    msg += "</script>"
